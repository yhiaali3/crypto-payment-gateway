...
- Backend: Express API في `server/` مع MongoDB للتخزين


## طبقات النظام
- Presentation: `client/` — صفحات الواجهة، صفحات الدفع، لوحة التاجر.
- API Layer: `server/routes/*` — نقاط النهاية التي تتعامل مع الطلبات.
- Services: `server/services/*` — طبقة منطق الأعمال التي تتعامل مع بوابات الدفع، توليد العناوين، والتحقق من المدفوعات.
- Data Layer: `server/models/*` — تفاعلات مع MongoDB.
